...
SELECT COUNT(E.EMPLOYEE_ID) "Nro Empleados"
    ...
    ,TO_CHAR(MAX(E.SALARY*(1+NVL(E.COMMISSION_PCT,0))), '999,999.999U') "Salario" -- No entiendo el max
FROM EMPLOYEES E
    INNER JOIN EMPLOYEES M ON E.MANAGER_ID=M.EMPLOYEE_ID
    INNER JOIN DEPARTMENTS D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID
...
WHERE M.FIRST_NAME || ' ' || M.LAST_NAME IN ('Steven King','Payam Kaufling')
    AND E.HIRE_DATE > '12-DEC-07'
GROUP BY R.REGION_NAME --No entiendo porque se usa solo el Nombre de la Region
ORDER BY "Nro Empleados" DESC
    , "Salario" DESC
;
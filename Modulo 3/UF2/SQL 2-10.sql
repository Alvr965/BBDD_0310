...
FROM EMPLOYEES E
    INNER JOIN DEPARTMENTS D USING (DEPARTMENT_ID)
    LEFT OUTER JOIN EMPLOYEES M ON D.MANAGER_ID=M.EMPLOYEE_ID
GROUP BY DEPARTMENT_NAME
ORDER BY 1;
...
9
SELECT DISTINCT DEPARTMENT_NAME "Dpto."
FROM DEPARTMENTS
    NATURAL JOIN EMPLOYEES
WHERE EMPLOYEES.JOB_ID IN ('SA_MAN','SA_REP')
ORDER BY 1;

10
SELECT COUNT(E.EMPLOYEE_ID) "Nro Empleados"
    , TO_CHAR(SUM(E.SALARY*(1+NVL(E.COMMISSION_PCT,0))), '999,999') "Salario"
    , R.REGION_NAME "Región"
FROM EMPLOYEES E
    INNER JOIN EMPLOYEES M ON E.MANAGER_ID=M.EMPLOYEE_ID
    INNER JOIN DEPARTMENTS D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID
	...
	CROSS JOIN EMPLOYEES K
WHERE K.FIRST_NAME || ' ' || K.LAST_NAME = 'Ki Gee'
  AND E.HIRE_DATE > K.HIRE_DATE
  AND M.FIRST_NAME || ' ' || M.LAST_NAME IN ('Steven King','Payam Kaufling')
